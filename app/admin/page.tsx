    ...
    <main className="min-h-screen bg-zinc-50">
      <div className="max-w-5xl mx-auto px-6 py-12">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          ...
          <Link
            href="/"
            className="inline-flex items-center justify-center px-4 py-2 rounded-lg border border-zinc-300 bg-white text-sm font-medium hover:bg-zinc-100 transition-colors"
          >
            Back to Portfolio
          </Link>
        </div>

        <div className="bg-white border border-zinc-200 rounded-2xl p-4 md:p-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
            <div className="text-sm text-zinc-600">
              {isFirebaseConfigured() ? (
                <span>
                  CMS: <span className="font-medium text-zinc-900">Firebase</span>{" "}
                  {user ? (
                ...
                <span>
                  CMS: <span className="font-medium text-zinc-900">Not configured</span>
                </span>
              )}
              ...
              {isFirebaseConfigured() ? (
                user ? (
                  <button
                    ...
                    className="px-3 py-2 rounded-lg border border-zinc-300 bg-white text-sm font-medium hover:bg-zinc-100 transition-colors"
                  >
                    Sign out
                  </button>
                ) : (
          ...
          <textarea
            ...
            className="w-full h-[60vh] rounded-xl border border-zinc-300 bg-zinc-50 p-4 font-mono text-sm text-zinc-800 outline-none focus:ring-2 focus:ring-blue-600"
            spellCheck={false}
            disabled={!isReady}
          />
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            <button
              onClick={handleReset}
              className="px-4 py-2 rounded-lg border border-zinc-300 bg-white text-sm font-medium hover:bg-zinc-100 transition-colors"
            >
              Reset to Default
            </button>
          </div>
